...
```
- 관리 버튼 클릭 > 설정메뉴 선책 > Format On Save 검색
...
# Was(Web Server Applocation Service)

# Was(Web Applocation Server)
...
common.css(공통적용), hearder.css, footer.css 등 영역별로 생성
```

- `프로젝트명/js 폴더` 생성

```
각 종류의 라이브러리. js, header.js, footer.js 영역별로 생성
```
...
- Git-2.49.0-64-bit Git for Windows Setup

### 1.1. 설치 확인 방법

- '윈도우 로고 + R키입력' > 'cmd로 터미널 실행'
...
- Git Bash 를 선택한 이유는 Linux, Mac 운영체제의 터미널

```
관리도구 버튼 > Settings > 입력창: defauli: Windows > Git Bash 선택
```

- 설정확인

```
VSCode 주메뉴 > Terminal > Terminal 보기 선택 후 Bash 확인 Ctrl+`
```
...
### 1.4. Enter 키에 의한 줄내림 동일하기(Linux, Mac, Window)

```bash
git config --global core.autocrlf true
```

### 1.5. Git 명령의 추가 내용 작성을 VSCode 하도록 설정.
